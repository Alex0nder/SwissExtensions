...
(function () {
  ...
  async function downloadPng() {
    savePngFormat();
    const base = getFileBase(pageInfo);
    const isWhole = pngFormat === 'whole';
    ...
    try {
      if (isWhole) {
        ...
        if (pickedDirHandle) {
          ...
          setDownloadStatus(`Saved ${tiles.length} files.`, 'done');
          setDownloadProgress(tiles.length, tiles.length);
        } else {
          tiles.forEach((dataUrl, i) => {
          ...
          setDownloadProgress(tiles.length, tiles.length);
          setDownloadStatus(`Download of ${tiles.length} files.`, 'done');
        }
      }
    } catch (e) {
      ...
      if (downloadStatusEl.className.includes('done')) {
